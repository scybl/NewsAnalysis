# Notice

ValueScope / NewsAnalysis is a personal financial data platform created by Libingze Li.

This public portfolio package is intentionally incomplete. It exists to demonstrate product thinking, architecture, system design, data governance, and engineering maturity without exposing private implementation details.

## Important Boundaries

- This package is not an open source release of the production system.
- The production repository, runtime data, credentials, deployment scripts, crawler logic, and cold backup implementation are private.
- Any resemblance between the mock examples and production data is illustrative only.
- No financial advice is provided by this package.

## Attribution

If you reference this project in an interview, review, article, or internal hiring note, please attribute it as:

`ValueScope / NewsAnalysis by Libingze Li`

## Contact

For source-level review, request a private screen-share or temporary read-only access from the author.
